...
:: =========================================================
::  LAUDO COMPLETO DE REDE v2001 - GPON/ONU/ONT/Wi-Fi 2.4G-5G
::  Criado por: Ronald
...
title LAUDO DE REDE v2001 - por Ronald
color 0A
set "AQUI=%~dp0"
...
echo.
echo   [TESTE] Rodando a versao 35 baixada do GitHub - update OK!
timeout /t 3 >nul
...
set "TECNICO=%TEC_OK%"
title LAUDO DE REDE v34 - Tecnico: %TECNICO%
...
:: =========================================================
:: LAUDO TECNICO + VOZ + TELEGRAM (falar_status.ps1)  [v34]
...
echo if ($resp -gt 0) { $st = $lat ^| Measure-Object -Minimum -Maximum -Average; $pminA[$i] = [int]$st.Minimum; $pmaxA[$i] = [int]$st.Maximum; $pmedA[$i] = [math]::Round($st.Average); $jitA[$i] = $pmaxA[$i] - $pminA[$i] } >>"%PFALA%"
...
echo $tg += "==== DIAGNOSTICO ====" >>"%PFALA%"
echo foreach ($x in $dg) { $tg += $x } >>"%PFALA%"
echo $tg += "" >>"%PFALA%"
echo $tg += "VEREDITO: " + $curto >>"%PFALA%"
...
echo Write-Host "" >>"%PTG%"
echo Write-Host "Enviando laudo para o Telegram...  [MODULO TELEGRAM v34]" -ForegroundColor Cyan >>"%PTG%"
...
echo Write-Host ("       LAUDO DE REDE v34 - Tecnico: " + $tec) >>"%PWAIT%"
echo Write-Host "=====================================================" >>"%PWAIT%"
...
echo   Deseja atualizar agora?
echo     [S] Sim, baixar e reiniciar
echo     [N] Nao, continuar nesta versao
echo.
set "RESP_UP="
set /p "RESP_UP=  Escolha S ou N: "
if /i not "%RESP_UP%"=="S" (echo   Mantendo a versao atual. & timeout /t 1 >nul & goto :eof)
